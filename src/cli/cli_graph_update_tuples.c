...
void
cli_graph_update_undirected_tuples(int old_schema_size);

void
cli_graph_update_tuples(schema_type_t st, int old_schema_size)
{
	if(st != VERTEX)
		cli_graph_update_undirected_tuples( old_schema_size);
...
//need to call this since we have 2 times as many back edges that need new schema info
void
cli_graph_update_undirected_tuples(int old_schema_size)
{
		int efd, fd, b_fd1, b_fd2;
	int new_schema_size;
	int readlen, writelen;
	ssize_t len, back_len;
	enum_list_t el = NULL;
	schema_t schema = NULL;
	attribute_t attr = NULL;
	char s[BUFSIZE], s1[BUFSIZE], b[BUFSIZE], b1[BUFSIZE];
	...
	/* Open the back edge for reading */
	memset(s, 0, BUFSIZE);
	sprintf(s, "%s/%d/%d/undir_e", grdbdir, gno, cno);
#if _DEBUG
	printf("cli_graph_update_undirected_tuples: ");
	printf("current vertex file %s\n", s);
#endif
	b_fd1 = open(s, O_RDONLY);

	/* Open new back edge for writing */
	memset(s, 0, BUFSIZE);
	sprintf(s, "%s/%d/%d/undir_e.new",
		grdbdir, gno, cno);
#if _DEBUG
	printf("cli_graph_update_undirected_tuples: ");
	printf("new vertex file %s\n", s);
#endif
	b_fd2 = open(s, O_WRONLY | O_CREAT, 0644);
	...
	for (;;) {
		...
		readlen = old_schema_size + (sizeof(vertexid_t) << 1);

		len = read(b_fd1, s, readlen);
		if (len < readlen) 
		{
			if (len == 0)
				break;
	...
	/* Move the vertex to a temporary file name */
	memset(s, 0, BUFSIZE);
	sprintf(s, "%s/%d/%d/undir_e",
		grdbdir, gno, cno);
	memset(s1, 0, BUFSIZE);
	sprintf(s1, "%s/%d/%d/undir_e.tmp",
		grdbdir, gno, cno);
	if (rename(s, s1) < 0) {
#if _DEBUG
		printf("cli_graph_update_tuples: ");
